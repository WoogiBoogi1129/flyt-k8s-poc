...
| D3 | MLP 실행 시 필요한 Runtime registration/launch·라이브러리 API를 추적하고 지원. `cudaLaunchKernel`의 현재 not-supported와 관련 누락 해소 | 같은 PyTorch 빌드에서 forward/backward/SGD, 1/100 step 결과, 지원 API와 변경 목록 |
...
단독 장치로 관측됐다. 당시 드라이버는 nvidia였다. 이 사실은 VFIO 전환 완료를
뜻하지 않는다. 전환 시 HAMi 등록/예약과 실제 GPU 사용을 다시 확인해야 한다.

현재 정상 회수 문서에 기록된 시험용 finalizer 보조 성공을 D4 통과로 승계하지 않는다.
